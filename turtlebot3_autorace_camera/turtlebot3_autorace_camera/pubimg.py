...
def keep_largest_component(mask):
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15,15))  # 커널 크기 조정 가능
...
def fit_and_draw_lanes(image, white_mask, yellow_mask, lane_node):
    ...
    lx = fit_curve(yellow_mask)
    rx = fit_curve(white_mask)
    if lx is not None:
    ...
    if lx is not None and rx is not None:
        cx = (lx + rx)/2
        pts = np.vstack([cx, ploty]).T.astype(np.int32)[None,:,:]
        cv2.polylines(out, pts, False, (0,255,255), 3)
        lane_node.publish_center(cx[int(h/2)])
    return out
...
def process_and_visualize(image, hsv, wh_range, yh_range, lane_node):
    ...
    # # White Count 기준 조정
    # if white_count >= 35000:
    #     new_w_vmin = min(current_w_vmin + 2, V_MIN_UPPER)
    # elif white_count <= 20000:
    ...
    # # Yellow Count 기준 조정
    # if yellow_count >= 35000:
    #     new_y_vmin = min(current_y_vmin + 2, V_MIN_UPPER)
    # elif yellow_count <= 20000:
    #     new_y_vmin = max(current_y_vmin - 2, V_MIN_LOWER)
    # else:
