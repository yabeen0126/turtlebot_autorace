...
class DetectTrafficLight(Node):
    ...
    def __init__(self):
        ...
        self.pub_traffic_status = self.create_publisher(String, '/traffic_light/status', 10)
    ...
    def find_traffic_light(self):
        ...
        if detect_red:
            ...
            msg.data = 'NONE'
        self.pub_traffic_status.publish(msg)
    ...
    def find_circle_of_traffic_light(self, mask, color):
        ...
        # ROI 시각화 (디버깅용)
        cv2.imshow(f'ROI - {color}', roi_mask)
        cv2.waitKey(1)

        for i in range(len(keypts)):
            self.point_x = int(keypts[i].pt[0])
            self.point_y = int(keypts[i].pt[1])
            size = keypts[i].size  # 지름 (diameter)
            radius = size / 2

            # 원 면적과 둘레를 계산하여 circularity 측정
            area = np.pi * (radius ** 2)
            perimeter = 2 * np.pi * radius
            circularity = (4 * np.pi * area) / (perimeter ** 2)

            self.get_logger().info(
            f'Detected {color} blob at ({self.point_x}, {self.point_y}) | '
            f'Size: {size:.1f} px | Radius: {radius:.1f} px | '
